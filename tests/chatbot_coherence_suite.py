...
class ChatbotCoherenceTests(unittest.TestCase):
    maxDiff = None
    ...
    def setUpClass(cls) -> None:
        ...
        if not api_key:
            raise unittest.SkipTest("OPENAI_API_KEY is required for coherence evaluation.")
    ...
    def test_coherence_cases(self) -> None:
        ...
        for case in CASES:
            with self.subTest(case=case.name):
                ...
                self.assertTrue(answer.strip(), "The chatbot answer must not be empty before coherence evaluation.")
                exchanges.append({"question": case.question, "answer": answer})

        evaluation = self._judge_coherence(exchanges)
    ...
    def _judge_coherence(self, exchanges: list[dict[str, str]]) -> dict[str, Any]:
        system_prompt = (
            "Tu es un evaluateur de coherence redactionnelle pour un chatbot de site web en francais. "
            "Tu juges la logique interne des reponses, leur fluidite, l'absence de contradictions et la clarte globale. "
        ...
        user_prompt = (
            ...
            "Evalue uniquement la coherence globale des reponses.\n\n"
            + "\n\n".join(
        ...
        try:
            ...
            self.fail(f"Coherence judge did not return valid JSON. Raw content: {content!r}. Error: {exc}")
